...
declare -r JOB_ID="Thirdeye_${USER}_$(date +%Y%m%d_%H%M%S)"
declare -r BUCKET="gs://${PROJECT}-ml"
declare -r GCS_PATH="${BUCKET}/${USER}/${JOB_ID}"
declare -r DICT_FILE=gs://pubsubdata/trainset/dict.txt

declare -r MODEL_NAME=Testing3
...
python trainer/preprocess.py \
  ...
  --input_path "gs://pubsubdata/trainset/eval_set.csv" \
  --output_path "${GCS_PATH}/preproc/eval" \
  --cloud

python trainer/preprocess.py \
  --input_dict "$DICT_FILE" \
  --input_path "gs://pubsubdata/trainset/train_set.csv" \
